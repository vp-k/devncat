...
This is `devncat` — a **Claude Code plugin marketplace** that bundles eight plugins as git submodules under `plugins/`. It is NOT a Flutter application itself; it provides Claude Code skills/commands for Flutter development and other workflows.
...
```
.claude-plugin/marketplace.json   # Marketplace manifest (lists 8 plugins)
plugins/
...
└── web-pipeline/                  # Git submodule → vp-k/web-pipeline
```
